...
test_loss_select=0
Dir='Model_GLU_Y_X/'
loss_folder='Model_GLU_Y_X/loss_Y_to_X_GLU/'
model_pkl_folder='Model_GLU_Y_X/model_pkl_Y_to_X_GLU/'
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")#device = torch.device('cpu')#('cuda:0')
print(device)
count_filename = "count_Y_to_X_GLU.txt"
# 如果文件不存在，则初始化计数器为0
...
if not os.path.isfile('model_Y_to_X_GLU.txt'):
    open('model_Y_to_X_GLU.txt', 'w').close()
if count>=1:
    # 读取 model.txt 文件的最后一行，获取上次运行的 pkl 文件名
    with open('model_Y_to_X_GLU.txt', 'r') as f:
        linescc = f.readlines()
        if linescc:
...
for epoch in range(epochs):
    for batch_idx in tqdm(range(len(train)), desc='train'):
        ...
        if data.size()[0] == predict_the.size()[0]:
            ...
            output = net_glu_96_5(data,device).requires_grad_(True).to(device)
    ...
    for batch_idx in tqdm(range(len(test)), desc='test'):
        ...
        if test_data.size()[0] == predict_the.size()[0]:
            ...
            output = net_glu_96_5(test_data,device).requires_grad_(True).to(device)
    ...
    if(test_loss_select>temp):
        ...
        torch.save(net_glu_96_5.state_dict(), model_pkl_folder+'GLU_96_5'+ str(learning_rate)+'_' + str(batch_size)+'_' + str(epochs)+'_' + str(count) +'.pkl')
...
with open('model_Y_to_X_GLU.txt', 'a') as f:
        f.write(model_pkl_folder+'GLU_96_5' + str(learning_rate)+'_' + str(batch_size)+'_' + str(epochs)+'_' + str(count) +'.pkl\n')
