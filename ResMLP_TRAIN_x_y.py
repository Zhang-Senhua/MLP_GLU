...
hidden_size=512#隐藏层的节点数量
hidden_size=torch.tensor(hidden_size)
num_layers=5#隐藏层数量
num_layers=torch.tensor(num_layers)
test_loss_select=0
Dir='Model_RES_X_Y/'
loss_folder='Model_RES_X_Y/loss_X_to_Y_RES/'
model_pkl_folder='Model_RES_X_Y/model_pkl_X_to_Y_RES/'
...
print(device)
count_filename = "count_X_to_Y_RES.txt"
# 如果文件不存在，则初始化计数器为0
...
if not os.path.isfile('model_X_to_Y_RES.txt'):
    open('model_X_to_Y_RES.txt', 'w').close()
if count>=1:
...
net_RES_5_96= ResMLP(input_size, output_size, hidden_size, num_layers,device).to(device) #96->5
if count>=1:
...
for epoch in range(epochs):
    for batch_idx in tqdm(range(len(train)), desc='train'):
        ...
        if data.size()[0] == predict_the.size()[0]:
            ...
            output = net_RES_5_96(target,device).requires_grad_(True).to(device)
    ...
    for batch_idx in tqdm(range(len(test)), desc='test'):
        ...
        if test_data.size()[0] == predict_the.size()[0]:
            ...
            output = net_RES_5_96(test_target,device).requires_grad_(True).to(device)
    ...
    if(test_loss_select>temp):
        ...
        torch.save(net_RES_5_96.state_dict(), model_pkl_folder+'RES_5_96'+ str(learning_rate)+'_' + str(batch_size)+'_' + str(epochs)+'_' + str(count) +'.pkl')
...
#         torch.save(net_RES_5_96.state_dict(), 'model_pkl/finlc'+str(learning_rate)+'_' + str(batch_size)+'_' + str(epochs)+'_' + str(count)+'.pkl')
...
with open('model_X_to_Y_RES.txt', 'a') as f:
        f.write(model_pkl_folder+'RES_5_96' + str(learning_rate)+'_' + str(batch_size)+'_' + str(epochs)+'_' + str(count) +'.pkl\n')
