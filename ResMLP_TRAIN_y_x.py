...
test_loss_select=0
Dir='Model_RES_Y_X/'
loss_folder='Model_RES_Y_X/loss_Y_to_X_RES/'
model_pkl_folder='Model_RES_Y_X/model_pkl_Y_to_X_RES/'
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")#device = torch.device('cpu')#('cuda:0')
print(device)
count_filename = "count_Y_to_X_RES.txt"
# 如果文件不存在，则初始化计数器为0
...
if not os.path.isfile('model_Y_to_X_RES.txt'):
    open('model_Y_to_X_RES.txt', 'w').close()
if count>=1:
...
net_RES_96_5= ResMLP(input_size, output_size, hidden_size, num_layers,device).to(device) #96->5
if count>=1:
        net_RES_96_5 .load_state_dict(torch.load(last_linecc))

optimizer_c = optim.Adam(net_RES_96_5.parameters(),lr=learning_rate)#,weight_decay=0.005,weight_decay=0.5
...
for epoch in range(epochs):
    for batch_idx in tqdm(range(len(train)), desc='train'):
        ...
        if data.size()[0] == predict_the.size()[0]:
            ...
            output = net_RES_96_5(data,device).requires_grad_(True).to(device)
    ...
    for batch_idx in tqdm(range(len(test)), desc='test'):
        ...
        if test_data.size()[0] == predict_the.size()[0]:
            ...
            output = net_RES_96_5(test_data,device).requires_grad_(True).to(device)
    ...
    if(test_loss_select>temp):
        ...
        torch.save(net_RES_96_5.state_dict(), model_pkl_folder+'RES_96_5'+ str(learning_rate)+'_' + str(batch_size)+'_' + str(epochs)+'_' + str(count) +'.pkl')
...
#         torch.save(net_RES_96_5.state_dict(), 'model_pkl/finlc'+str(learning_rate)+'_' + str(batch_size)+'_' + str(epochs)+'_' + str(count)+'.pkl')
...
with open('model_Y_to_X_RES.txt', 'a') as f:
        f.write(model_pkl_folder+'RES_96_5' + str(learning_rate)+'_' + str(batch_size)+'_' + str(epochs)+'_' + str(count) +'.pkl\n')
